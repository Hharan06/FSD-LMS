...
exports.createCourse = async (req, res) => {
    try {
        // extract data
        let { courseName, courseDescription, whatYouWillLearn, price, instructions: _instructions, status, tag: _tag } = req.body;
        ...
        try {
            ...
            console.log('Thumbnail upload failed:', uploadError.message);
            
            // Use placeholder image as fallback
            thumbnailUrl = 'https://via.placeholder.com/300x200/4F46E5/FFFFFF?text=Course+Thumbnail';
            
            // Log the error but don't fail the course creation
            console.log('Using placeholder thumbnail due to upload failure');
        }
...
exports.getAllCourses = async (req, res) => {
    try {
        // Find all courses (including Draft status for now to debug)
        const allCourses = await Course.find({},
            {
                courseName: true, courseDescription: true, price: true, thumbnail: true, instructor: true,
                ratingAndReviews: true, studentsEnrolled: true, status: true, createdAt: true
            })
            .populate({
                path: 'instructor',
        ...
        console.log('Courses:', allCourses.map(c => ({ name: c.courseName, status: c.status })));
        
        // For debugging: show all courses regardless of status
        console.log('Sending all courses to frontend (including Draft)');
